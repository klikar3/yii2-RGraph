'version:2023-07-17 (6.13)';RGraph=window.RGraph||{isrgraph:true,isRGraph:true,rgraph:true};(function(win,doc,undefined)
...
var context=obj.context;var annotations=win.localStorage['__rgraph_annotations_'+obj.id+'__'];var i,len,move,coords;context.save();context.beginPath();context.lineWidth=obj.get('annotatableLinewidth');if(annotations&&annotations.length){annotations=annotations.split('|');}else{return;}
context.strokeStyle=annotations[i]||'black';for(i=0,len=annotations.length;i<len;++i){if(annotations[i].match(/[a-z]+/)){context.stroke();context.beginPath();context.strokeStyle=annotations[i];move=true;continue;}
...
context.stroke();context.restore();};window.addEventListener('load',function(e)
{setTimeout(function()
